...
你的記憶存放在 `.claude/memory-kb/database/`，透過 MCP 工具操作：
...
2. **理解數據** — 確認新數據的結構、來源、預期的查詢模式
3. **設計模型** — 設計 ORM 模型，規劃表結構、索引、關係
4. **實現代碼** — 編寫模型代碼和遷移腳本
...
運行中遇到任何錯誤或非預期行為時，**修復後必須**用 `write_note` 在 `database/` 資料夾新增一筆筆記：
