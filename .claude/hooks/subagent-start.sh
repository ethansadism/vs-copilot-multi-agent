...
try:
    ...
    with open(os.path.join(log_dir, "hook-flow.log"), "a") as f:
        f.write(f"[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [SubagentStart] >>> {agent_name} (ID: {agent_id}) spawned\n")
    ...
    agent_folder = folder_map.get(agent_name, "")
    ...
    # 記錄啟動時間戳（供 subagent-memory-check.sh 使用）
    ts_file = os.path.join(log_dir, f"subagent-start-{agent_id}.timestamp")
    with open(ts_file, "w") as f:
        f.write(datetime.datetime.now().isoformat())

    context = f"""## {agent_name} Basic Memory 已就緒 (Agent ID: {agent_id})

你的記憶資料夾: `{memory_kb}/{agent_folder}/`

### 現有筆記:
{notes_list if notes_list else "  （無）"}
...
### ⚠️ 重要：任務結束前必須用 write_note 更新記憶，否則 Stop hook 會阻擋你完成任務。
"""
